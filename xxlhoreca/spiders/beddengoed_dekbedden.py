...
class dekbedSpider(Spider):
    name = "beddengoed_dekbedden"
    start_urls = ['https://www.beddengoed.com/volwassenen/dekbedden', 'https://www.beddengoed.com/kinderen/dekbedden', 'https://www.beddengoed.com/baby-en-peuters/dekens-dekbedden-kussens']
    ...
    def finalParse(self, response):
        try:
            ...
            for option in options:
                if 'Type Dekbed:' in option:
                    item['Type Dekbed'] = option.split(':')[-1].strip()
                elif 'Tijk:' in option:
                    item['Tijk'] = option.split(':')[-1].strip()
                elif 'Vulling:' in option:
                    item['Vulling'] = option.split(':')[-1].strip()
                elif 'Warmteklasse' in option:
                    item['Warmteklasse '] = option
