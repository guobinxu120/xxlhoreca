...
class dekbedSpider(Spider):
    name = "leenbakker_boxsprings"
    start_urls = ['http://www.leenbakker.nl/bed-en-toebehoren/boxsprings']
    ...
    def parse(self, response):
        ...
        nexts_style = response.xpath('//a[contains(@id,"WC_SearchBasedNavigationResults_pagination_link_right_")]/@class').extract_first()

        if nexts and nexts_style != "right_arrow invisible":
    ...
    def finalParse(self, response):
        try:
            ...
            oldprices = price_tag[1].xpath('.//span[@class="old_price"]//text()').re(r'[\d,.]+')
            # oldprice = 0.0
            if oldprices:
                oldprice = '.'.join(oldprices).replace(' ', '')
            item['old_price'] = oldprice
            item['delivery'] = response.xpath('//span[@itemprop="availability"]/text()').extract_first().strip()
            ...
            for tr in lis:
                ...
                elif 'Type topdekmatras'  in key:
                    item['Type topdekmatras'] = val
                    # if not key in self.headers:
                    #     self.headers.append(key)
                elif 'Inclusief matras' in key:
                    item['Inclusief matras'] = val
                    # if not key in self.headers:
                    #     self.headers.append(key)
                elif 'Product afmetingen' == key:
                    item['Product afmetingen'] = val
                    # if not key in self.headers:
                    #     self.headers.append(key)
                # elif 'Materiaal' == key:
                #     item[key] = val
