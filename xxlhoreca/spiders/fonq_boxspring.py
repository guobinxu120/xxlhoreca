...
class dekbedSpider(Spider):
    name = "fonq_boxspring"
    start_urls = ['https://www.fonq.nl/producten/categorie-boxspring/?fq=boxspring']
    ...
    headers = ['name', 'brand', 'url', 'price', 'old_price', 'rating', 'reviews', 'delivery']
    def start_requests(self):
        yield Request(self.start_urls[0], callback=self.parse, meta={'page':1})
    def parse(self, response):
        products = response.xpath('//div[@class="col-sm-4 col-xxs-6 "]//div[@class="product-body"]/a[1]/@href').extract()
        ...
        try:
            ...
            for tr in trs:
                ...
                if 'Allergievrij' in key:
                    ...
                    if not key in self.headers:
                        self.headers.append(key)
                elif 'Gemonteerd bij levering' == key:
                    item[key] = val
                    if not key in self.headers:
                        self.headers.append(key)
                elif 'Max. gewicht' == key:
                    item[key] = val
                    if not key in self.headers:
                        self.headers.append(key)
