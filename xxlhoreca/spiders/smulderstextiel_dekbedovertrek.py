...
class dekbedSpider(Spider):
    name = "smulderstextiel_dekbedovertrek"
    start_urls = ['https://www.smulderstextiel.nl/dekbedovertrek/']
    ...
    def finalParse(self, response):
        # try:
            ...
            item['Instopstrook'] = ''.join(response.xpath('//dt[text()="Instopstrook"]//following-sibling::dd/div/div//text()').extract()).strip()
            item['Materiaal'] = ''.join(response.xpath('//dt[text()="Materiaal"]//following-sibling::dd/div/div//text()').extract()).strip()
